...
red_board = deepcopy(board)
blue_board = deepcopy(board)
empty_board = deepcopy(board)

# 다른 구슬이 없는 각자의 보드 만들기
red_board[blue[0]][blue[1]] = '.'
blue_board[red[0]][red[1]] = '.'
...
def move(rx, ry, bx, by, dir): # dir 0~3
    ...
    while True: # for red
        ...
        if rnx > 0 and rny > 0 and rnx < n-1 and rny < n-1 and rb[rnx][rny] != '#': 
            if rb[rnx][rny] == 'O':
                rhole = True
            else:
    ...
    while True: # for blue
        ...
        if bnx > 0 and bny > 0 and bnx < n-1 and bny < n-1 and bb[bnx][bny] != '#': 
            if bb[bnx][bny] == 'O':
                bhole = True
            else:
...
#     for dir in range(4):
#         move(dir)
#         arr = find(red_board)[0][0]
#         if not red_visited[arr[0]][arr[1]]:
#             red_visited[arr[0]][arr[1]] = True # 방문처리
#             q.append(arr) # 도착한곳 차례대로 append
#         # visited 
